...
func process(conn net.Conn) {
	...
	接收公钥
	*/
	DataOfPublicKey := ConveyUtils.RecvFrom(conn) //收到   '_over'  就停止接受的函数
	...
	*/
	...
	接收要处理的图片数据
	*/
	FirstData := ConveyUtils.RecvFrom(conn)
	...
	*/
	...
	接收要隐藏的信息
	*/
	SecondData := ConveyUtils.RecvFrom(conn)
	...
	*/
	...
	ConveyUtils.ConveyData(conn, ADDFirSec)
	ConveyUtils.ConveyData(conn, []byte("_over"))
}
